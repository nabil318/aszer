<?php // Démarrer la session pour stocker les réponses de l'utilisateur
session_start();
...
for ($i = 1; $i <= 8; $i++) { // Changer la boucle pour inclure DP_8
    ...
    if ($result && !empty($result['type'])) {
        ...
        // Modifier le type de question pour QM
        if ($result['type'] === 'QM') {
            $result['type'] = 'QROC';
        }
        $questions[$i] = $result;
...
<body>
            ...
            <?php foreach ($questions as $i => $question) { ?>
                <?php if ($i == 1) { ?>
                    <div class="accueil-section">
                        <h1>keys Feature Problems (KFP)</h1>
                ...
                <div class="question-div" id="question-<?php echo $i; ?>" data-correct="<?php echo htmlspecialchars($question['reponse_correcte']); ?>" data-zone="<?php echo htmlspecialchars($question['zone_correcte']); ?>" data-correct-count="<?php echo htmlspecialchars($question['correct_count']); ?>">
                    <div class="question-type"><?php echo $question['type']; ?></div> <!-- Ajouter le type de QCM ici -->
                    <div class="question-score hidden-correction" id="score-<?php echo $i; ?>">Score : </div>
                    ...
                    if ($type === 'QRU') {
                        ...
                        // Ajouter les boutons "Valider"
                        echo "<button type='button' class='validate-btn' data-current='$i' data-next='" . ($i + 1) . "'>Valider</button>";
                        // Ajouter le bouton "Ma note est différente" pour les questions de type QRM
                        echo "<button type='button' class='different-score-btn hidden' data-question='$i'>Ma note est différente</button>";
                        echo "<input type='number' class='hidden different-score-input' id='different-score-$i' min='0' max='1' step='0.01' placeholder='Entrez la note'>";
                    } elseif ($type === 'QROC') {
                        echo "<input type='text' name='reponse[$i]' class='default-input'>";
                        echo "<button type='button' class='validate-btn' data-current='$i' data-next='" . ($i + 1) . "'>Valider</button>";
                        echo "<button type='button' class='different-score-btn hidden correct-btn' data-question='$i'>Ma réponse était bonne</button>";
                        echo "<button type='button' class='different-score-btn hidden incorrect-btn' data-question='$i'>Ma réponse était mauvaise</button>";
                    } elseif ($type === 'QP') {
                        for ($j = 1; $j <= 12; $j++) {
                            $prop = $question["prop$j"] ?? null;
                            $commentaire_prop = $question["commentaire_prop{$j}_DP"] ?? null;
                        ...
                        // Ajouter les boutons "Valider" et "Ma note est différente"
                        echo "<button type='button' class='validate-btn' data-current='$i' data-next='" . ($i + 1) . "'>Valider</button>";
                        echo "<button type='button' class='different-score-btn hidden' data-question='$i'>Ma note est différente</button>";
                        echo "<input type='number' class='hidden different-score-input' id='different-score-$i' min='0' max='1' step='0.01' placeholder='Entrez la note'>";
                        // Ajouter l'input pour le nombre de réponses correctes
                        echo "<div class='hidden-correction'>Nombre de réponses correctes attendues : " . htmlspecialchars($question['correct_count']) . "</div>";
                        echo "<div class='hidden-correction' id='correct-answers-$i'></div>"; // Ajouter un div pour afficher les réponses correctes
                        echo "<div class='hidden-correction' id='user-answers-$i'></div>"; // Ajouter un div pour afficher les réponses de l'utilisateur
                        echo "<div class='hidden-correction' id='score-display-$i'></div>";
                    } else {
    ...
    <script>
        ...
        function validateQuestion(questionId) {
            ...
            const questionType = questionDiv.dataset.type; // Récupérer le type de question
            ...
            if (type === 'radio') {
                ...
                if (intolerableChecked || indispensableUnchecked) {
                    ...
                    const expectedCorrectCount = correctAnswers.length;
                    const userCorrectCount = userAnswers.filter(answer => correctAnswers.includes(answer)).length;
                    partialScore = userCorrectCount / expectedCorrectCount;
                }
            } else if (type === 'text') {
                // QROC : Question avec réponse textuelle
            ...
            document.getElementById('global-score').innerText = globalScore.toFixed(2);
        ...
        // Gestionnaires d'événements pour les boutons "Ma réponse était bonne" et "Ma réponse était mauvaise"
        document.querySelectorAll('.different-score-btn').forEach(button => {
            button.addEventListener('click', () => {
                const questionId = button.dataset.question;
                const inputField = document.querySelector(`#question-${questionId} input[type="text"]`);

                if (button.textContent.includes('bonne')) {
                    // Mettre à jour le style de l'input pour indiquer une réponse correcte
                    inputField.classList.add('correct1');
                    inputField.classList.remove('incorrect1');
                } else if (button.textContent.includes('mauvaise')) {
                    // Mettre à jour le style de l'input pour indiquer une réponse incorrecte
                    inputField.classList.add('incorrect1');
                    inputField.classList.remove('correct1');
                }
            });
        });

        // Gestionnaires d'événements pour les boutons "Ma réponse était bonne" et "Ma réponse était mauvaise" pour les questions de type QROC
        ...
        document.querySelectorAll('.incorrect-btn').forEach(button => {
            button.addEventListener('click', () => {
                const questionId = button.dataset.question;
                const scoreDiv = document.getElementById('score-' + questionId);
                scoreDiv.innerHTML = 'Score : <span class="correct">0/1</span>';
        ...
        document.getElementById('instant-correction-btn').addEventListener('click', () => {
            ...
            // Afficher ou masquer les éléments de correction
            document.querySelectorAll('.hidden-correction').forEach(element => {
                element.style.display = correctionVisible ? 'block' : 'none';
            });
            ...
            // Afficher ou masquer les boutons "Ma note est différente"
            document.querySelectorAll('.different-score-btn').forEach(button => {
        ...
        document.getElementById('final-submit').addEventListener('click', (event) => {
            ...
            // Rendre les boutons "Ma note est différente" visibles
            document.querySelectorAll('.different-score-btn').forEach(button => {
        ...
        // Gestionnaire d'événements pour le bouton "Ma note est différente"
        document.querySelectorAll('.different-score-btn').forEach(button => {
            button.addEventListener('click', () => {
                const questionId = button.dataset.question;
                const inputField = document.getElementById('different-score-' + questionId);
                const scoreDiv = document.getElementById('score-' + questionId);
                ...
                // Ajouter un gestionnaire d'événements pour mettre à jour le score lorsque l'utilisateur saisit une valeur
                inputField.addEventListener('input', () => {
                    const newScore = parseFloat(inputField.value);
                    if (!isNaN(newScore) && newScore >= 0 && newScore <= 1) {
                        questionScores[questionId] = newScore;
                        scoreDiv.innerHTML = 'Score : <span class="correct">' + newScore.toFixed(2) + '/1</span>';
