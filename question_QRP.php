...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($intolerableChecked || $indispensableUnchecked) {
        ...
        $correct_answers_count = count(array_intersect($reponses_correctes, $reponse_utilisateur_array));
        $total_expected_answers = count($reponses_correctes);
        $note = $total_expected_answers > 0 ? round(($correct_answers_count / $total_expected_answers) * 20, 2) : 0;
    }
    ...
    $stmt_insert->bindParam(':note', $note, PDO::PARAM_STR); // Enregistrer la note
    $stmt_insert->execute();
    ...
    <div id="question">
        ...
        <form method="POST">
            ...
            <?php if (isset($reponse_utilisateur)): ?>
                <div class="resultat">
                    <h3 class="invisible">Résultat :</h3>
                    <p class="invisible">Nombre de réponses attendues : <?php echo count($reponses_correctes); ?></p>
                    <p class="invisible">Réponses attendues : <?php echo implode(', ', $reponses_correctes); ?></p>
                    <p class="invisible">Réponses de l'utilisateur : <?php echo implode(', ', $reponse_utilisateur_array); ?></p>
                    <p class="invisible">Note : <?php echo $note; ?>/20</p>
                </div>
